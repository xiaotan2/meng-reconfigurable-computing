...
from BubbleSortRTL_arbiter import *
...
class TestHarness (Model):
  ...
  def __init__( s, xcel, arbiter, src_msgs, sink_msgs,
   ...
   # if dump_vcd:
    ...
    s.connect( s.arbiter.xcelreq,     s.xcel.memreq     )
    s.connect( s.arbiter.xcelresp,    s.xcel.memresp    )
    s.connect( s.arbiter.duaxcelreq,  s.xcel.duamemreq  )
    s.connect( s.arbiter.duaxcelresp, s.xcel.duamemresp )
    s.connect( s.arbiter.memreq,      s.mem.reqs[0]     )
    s.connect( s.arbiter.memresp,     s.mem.resps[0]    )
...
test_case_table = mk_test_case_table([
  ...
  [ "mini_0_8_4",         mini,           0,  8,   0.5,  4   ],
  [ "mini_8_0_0",         mini,           8,  0,   0.5,  0   ],
  [ "mini_8_0_4",         mini,           8,  0,   0.5,  4   ],
  [ "mini_8_8_0",         mini,           8,  8,   0.5,  0   ],
  [ "mini_8_8_4",         mini,           8,  8,   0.5,  4   ],
...
def run_test( xcel, arbiter, test_params, test_verilog=False ):# dump_vcd, test_verilog=False ):
  ...
  th = TestHarness( xcel, arbiter, xreqs, xresps,
...
def test( test_params ): #, dump_vcd ):
  run_test( BubbleSortRTL(), BSArbiter(), test_params ) #, dump_vcd )
