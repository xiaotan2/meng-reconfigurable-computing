...
class PolyDsuRbTreeInorderPipeline( Model ):
  ...
  def __init__( s ):
    ...
    def receive_response():
      ...
      s.rbtree_xcel.xcelresp.rdy.value = s.pipe_stg.curr_val & s.xcelresp.rdy

      s.xcelresp.val.value = s.pipe_stg.curr_val & s.rbtree_xcel.xcelresp.val

      s.pipe_stg.curr_stall.value = \
        s.pipe_stg.curr_val & ( ~s.xcelresp.rdy | ~s.rbtree_xcel.xcelresp.val )
  ...
  def line_trace( s ):
    return "{} {} | {} {}".format(
      s.rbtree_xcel.xcelreq,
      s.rbtree_xcel.xcelresp,
      s.memreq,
      s.memresp
    )
