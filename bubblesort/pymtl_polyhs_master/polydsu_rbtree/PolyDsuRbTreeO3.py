...
# PolyDsuRbTreeO3
#=========================================================================
# Top-level model that composes a PolyDsuRbTreeHLS model and a PolyDsuConfig
# model that tests the rbtree accelerator in isolation with the gem5 O3 model
...
from PolyDsuRbTreeHLS  import PolyDsuRbTreeHLS
...
class PolyDsuRbTreeO3( Model ):
  ...
  def __init__( s ):
    ...
    def xcel_connections():
      ...
      s.rbtree_hls.xcelreq.val.value   = 0
      s.rbtree_hls.xcelreq.msg.value   = 0
      ...
      if   s.xcelreq_q.deq.msg.id == s.DSTU_ID:
        ...
        s.rbtree_hls.xcelreq.val.value = s.xcelreq_q.deq.val
        s.rbtree_hls.xcelreq.msg.value = s.xcelreq_q.deq.msg
        s.xcelreq_q.deq.rdy.value    = s.rbtree_hls.xcelreq.rdy

      # xcelresp msg
      # NOTE: prioritize the cfg responses over the rbtree responses
      s.xcelresp.val.value            = 0
      s.xcelresp.msg.value            = 0
      s.config_hls.xcelresp.rdy.value = 0
      s.rbtree_hls.xcelresp.rdy.value   = 0
      ...
      elif s.rbtree_hls.xcelresp.val:
        s.xcelresp.val.value            = s.rbtree_hls.xcelresp.val
        s.xcelresp.msg.value            = s.rbtree_hls.xcelresp.msg
        s.rbtree_hls.xcelresp.rdy.value = s.xcelresp.rdy

    # mem connections
    s.connect( s.rbtree_hls.memreq,  s.rbtree_lsq.memreq_in  )
    ...
    # rbtree xcel to the response val-rdy bundle of the rbtree lsq.
    # XXX: Need to investigate why
    s.byp_q = SingleElementBypassQueue( MemMsg(8,32,32).resp )

    s.connect( s.rbtree_lsq.memresp_in, s.byp_q.enq )
    s.connect( s.rbtree_hls.memresp,    s.byp_q.deq )

    # lsq <-> memreq/memresp
    s.connect( s.memreq,        s.rbtree_lsq.memreq_out  )
    s.connect( s.memresp_q.deq, s.rbtree_lsq.memresp_out )

    # commit port connections
    s.connect( s.commitreq_q.deq, s.rbtree_lsq.commitreq  )
    s.connect( s.commitresp,      s.rbtree_lsq.commitresp )
