...
from PolyDsuListO3          import PolyDsuListO3
...
class PolyDsuListO3Pipeline( Model ):
  ...
  def __init__( s ):
    ...
    def receive_response():
      ...
      s.list_xcel.xcelresp.rdy.value = s.pipe_stg.curr_val & s.xcelresp.rdy & s.pipe.in_.rdy

      # cannot assert the valid signal until the pipe can accept a transaction
      s.xcelresp.val.value = s.pipe_stg.curr_val & s.list_xcel.xcelresp.val & s.pipe.in_.rdy
    ...
    def commit():
      ...
      s.list_xcel.commitreq.msg.value = commit_msg
      ...
      if (   ( s.pipe.out.msg.opc == PolyDsuReqMsg.TYPE_SET )
        ...
        s.list_xcel.commitreq.val.value = s.pipe.out.val
        s.pipe.out.rdy.value            = s.list_xcel.commitreq.rdy
      else:
        s.list_xcel.commitreq.val.value = 0
        s.pipe.out.rdy.value            = 1

      # tie the value of the response to be 1 always
      s.list_xcel.commitresp.rdy.value = 1
  ...
  def line_trace( s ):
    return "{} {} | ( {} {} ) | {} {}".format(
      s.xcelreq,
      s.xcelresp,
      s.list_xcel.commitreq,
      s.list_xcel.commitresp,
      s.memreq,
      s.memresp
    )
