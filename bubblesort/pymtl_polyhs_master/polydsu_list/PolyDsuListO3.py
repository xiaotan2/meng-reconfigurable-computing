...
# PolyDsuListO3
#=========================================================================
# Top-level model that composes a PolyDsuListHLS model and a PolyDsuConfig
# model that tests the list accelerator in isolation with the gem5 O3 model
...
from PolyDsuListHLS  import PolyDsuListHLS
...
class PolyDsuListO3( Model ):
  ...
  def __init__( s ):
    ...
    # list xcel
    s.list_hls   = PolyDsuListHLS()

    # list lsq
    s.list_lsq   = PolyDsuLSQ( 8 )
    ...
    def xcel_connections():
      ...
      s.list_hls.xcelreq.val.value   = 0
      s.list_hls.xcelreq.msg.value   = 0
      ...
      if   s.xcelreq_q.deq.msg.id == s.DSTU_ID:
        ...
        s.list_hls.xcelreq.val.value = s.xcelreq_q.deq.val
        s.list_hls.xcelreq.msg.value = s.xcelreq_q.deq.msg
        s.xcelreq_q.deq.rdy.value    = s.list_hls.xcelreq.rdy

      # xcelresp msg
      # NOTE: prioritize the cfg responses over the list responses
      s.xcelresp.val.value            = 0
      s.xcelresp.msg.value            = 0
      s.config_hls.xcelresp.rdy.value = 0
      s.list_hls.xcelresp.rdy.value   = 0
      ...
      elif s.list_hls.xcelresp.val:
        s.xcelresp.val.value            = s.list_hls.xcelresp.val
        s.xcelresp.msg.value            = s.list_hls.xcelresp.msg
        s.list_hls.xcelresp.rdy.value   = s.xcelresp.rdy

    # mem connections
    s.connect( s.list_hls.memreq,  s.list_lsq.memreq_in  )
    ...
    # list xcel to the response val-rdy bundle of the list lsq.
    # XXX: Need to investigate why
    s.byp_q = SingleElementBypassQueue( MemMsg(8,32,32).resp )

    s.connect( s.list_lsq.memresp_in, s.byp_q.enq )
    s.connect( s.list_hls.memresp,    s.byp_q.deq )

    # lsq <-> memreq/memresp
    s.connect( s.memreq,        s.list_lsq.memreq_out  )
    s.connect( s.memresp_q.deq, s.list_lsq.memresp_out )

    # commit port connections
    s.connect( s.commitreq_q.deq, s.list_lsq.commitreq  )
    s.connect( s.commitresp,      s.list_lsq.commitresp )
