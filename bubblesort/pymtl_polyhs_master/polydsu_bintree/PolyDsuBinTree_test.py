...
class TestHarness( Model ):
  ...
  def __init__( s, model,
    ...
    # Instantiate Models
    s.src   = TestSource ( PolyDsuReqMsg(), src_msgs, src_delay )
    s.dut   = model()
    s.sink  = TestSink ( PolyDsuRespMsg(), sink_msgs, sink_delay )
    s.mem   = TestMemory ( mem_ifc, 1, stall_prob, latency )

    # Connect
    s.connect( s.dut.xcelreq,  s.src.out  )
    s.connect( s.dut.xcelresp, s.sink.in_ )

    s.connect( s.dut.memreq,  s.mem.reqs[0]  )
    s.connect( s.dut.memresp, s.mem.resps[0] )
...
# run_test
#-------------------------------------------------------------------------
def run_test( model, mem_array, test_params, dump_vcd = None ):
...
bintree_int_msgs = [
  ...
  dsu_req_wr ( 0x400,  0,   32,    5 ), dsu_resp_wr ( 0x400,  0,  32,   0 ),
  dsu_req_wr ( 0x400,  0,   48,    6 ), dsu_resp_wr ( 0x400,  0,  48,   0 ),
  dsu_req_wr ( 0x400,  0,   64,    7 ), dsu_resp_wr ( 0x400,  0,  64,   0 ),
  dsu_req_wr ( 0x400,  0,   80,    8 ), dsu_resp_wr ( 0x400,  0,  80,   0 ),
...
insert_msgs = [
  ...
  # 3->1
  xcel_req   ( 0x400,  0, MTX,   1,    64 ), xcel_resp   ( 0x400,   0, MTX,     0 ), # header
  xcel_req   ( 0x400,  0, MTX,   2,    32 ), xcel_resp   ( 0x400,   0, MTX,     0 ), # node mem
  xcel_req   ( 0x400,  0, MTX,   3,     1 ), xcel_resp   ( 0x400,   0, MTX,     0 ), # val
  xcel_req   ( 0x400,  0, MFX,   0,     0 ), xcel_resp   ( 0x400,   0, MFX,    32 ), # result = mem

  # 3->5
  xcel_req   ( 0x400,  0, MTX,   1,    64 ), xcel_resp   ( 0x400,   0, MTX,     0 ), # header
  xcel_req   ( 0x400,  0, MTX,   2,    96 ), xcel_resp   ( 0x400,   0, MTX,     0 ), # node mem
  xcel_req   ( 0x400,  0, MTX,   3,     5 ), xcel_resp   ( 0x400,   0, MTX,     0 ), # val
  xcel_req   ( 0x400,  0, MFX,   1,     0 ), xcel_resp   ( 0x400,   0, MFX,    96 ), # result = mem

  # 1->2
  xcel_req   ( 0x400,  0, MTX,   1,    32 ), xcel_resp   ( 0x400,   0, MTX,     0 ), # header
  xcel_req   ( 0x400,  0, MTX,   2,    48 ), xcel_resp   ( 0x400,   0, MTX,     0 ), # node mem
  xcel_req   ( 0x400,  0, MTX,   3,     2 ), xcel_resp   ( 0x400,   0, MTX,     0 ), # val
  xcel_req   ( 0x400,  0, MFX,   1,     0 ), xcel_resp   ( 0x400,   0, MFX,    48 ), # result = mem

  # 5->4
  xcel_req   ( 0x400,  0, MTX,   1,    96 ), xcel_resp   ( 0x400,   0, MTX,     0 ), # header
  xcel_req   ( 0x400,  0, MTX,   2,    80 ), xcel_resp   ( 0x400,   0, MTX,     0 ), # node mem
  xcel_req   ( 0x400,  0, MTX,   3,     4 ), xcel_resp   ( 0x400,   0, MTX,     0 ), # val
  xcel_req   ( 0x400,  0, MFX,   0,     0 ), xcel_resp   ( 0x400,   0, MFX,    80 ), # result = mem

  # 5->6
  xcel_req   ( 0x400,  0, MTX,   1,    96 ), xcel_resp   ( 0x400,   0, MTX,     0 ), # header
  xcel_req   ( 0x400,  0, MTX,   2,   112 ), xcel_resp   ( 0x400,   0, MTX,     0 ), # node mem
  xcel_req   ( 0x400,  0, MTX,   3,     6 ), xcel_resp   ( 0x400,   0, MTX,     0 ), # val
  xcel_req   ( 0x400,  0, MTX,   4,     1 ), xcel_resp   ( 0x400,   0, MTX,     0 ), # left/right
  xcel_req   ( 0x400,  0, MFX,   1,     0 ), xcel_resp   ( 0x400,   0, MFX,   112 ), # result = mem
...
def test( test_params, dump_vcd ):
  run_test( TestHarness( PolyDsuBinTree,
                         test_params.msgs[::2],
