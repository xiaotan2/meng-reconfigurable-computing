...
from PolyDsuBinTreeO3        import PolyDsuBinTreeO3
...
class PolyDsuBinTreeO3Pipeline( Model ):
  ...
  def __init__( s ):
    ...
    # bintree xcel
    s.bintree_xcel = PolyDsuBinTreeO3()
    ...
    def receive_response():
      ...
      s.bintree_xcel.xcelresp.rdy.value = s.pipe_stg.curr_val & s.xcelresp.rdy & s.pipe.in_.rdy

      # cannot assert the valid signal until the pipe can accept a transaction
      s.xcelresp.val.value = s.pipe_stg.curr_val & s.bintree_xcel.xcelresp.val & s.pipe.in_.rdy
    ...
    def commit():
      ...
      s.bintree_xcel.commitreq.msg.value = commit_msg
      ...
      if (   ( s.pipe.out.msg.opc == PolyDsuReqMsg.TYPE_SET )
        ...
        s.bintree_xcel.commitreq.val.value = s.pipe.out.val
        s.pipe.out.rdy.value            = s.bintree_xcel.commitreq.rdy
      else:
        s.bintree_xcel.commitreq.val.value = 0
        s.pipe.out.rdy.value              = 1

      # tie the value of the response to be 1 always
      s.bintree_xcel.commitresp.rdy.value = 1
  ...
  def line_trace( s ):
    return "{} {} | ( {} {} ) | {} {}".format(
      s.xcelreq,
      s.xcelresp,
      s.bintree_xcel.commitreq,
      s.bintree_xcel.commitresp,
      s.memreq,
      s.memresp
    )
