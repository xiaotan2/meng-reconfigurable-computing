...
#====================================================================
from pymtl      import *
from pclib.rtl  import LtComparator, RegEnRst, RegRst, Mux
...
class FindMinDpathRTL( Model ):
  ...
  def __init__( s, nbits = 8, k = 10 ):
    ...
    s.knn_LtComparator = m = LtComparator( nbits )
    s.connect_dict({
      m.in0 : s.knn_data0,
      m.in1 : s.knn_data1,
      m.out : s.isSmaller
...
#====================================================================
# FindMin Control Unit
#====================================================================
class FindMinCtrlRTL( Model ):
...
# FindMin Top Level
#====================================================================
class FindMinPRTL( Model ):

  # Constructor
  def __init__( s, nbits = 8, k = 10 ):
    
    # Interface
    s.req  = InValRdyBundle  ( FindMinReqMsg()  ) 
    s.resp = OutValRdyBundle ( FindMinRespMsg() )

    # Instantiate datapath and control
    s.dpath = FindMinDpathRTL( nbits, k  )
    s.ctrl  = FindMinCtrlRTL ( nbits, k  )
    ...
    s.connect( s.dpath.resp_msg_idx,   s.resp.msg.digit   )                                                      
    
    s.connect( s.ctrl.req_rdy,        s.req.rdy           )
    s.connect( s.ctrl.resp_val,       s.resp.val          )

    # connect dpath/ctrl
    s.connect( s.dpath.isSmaller,  s.ctrl.isSmaller    )
